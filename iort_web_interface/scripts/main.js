...
function connectMQTT() {
  ...
  mqttClient.on("message", function (topic, message) {
    // Messaggi ricevuti (buffer → stringa)
    ...
    try {
      ...
      // Gestione messaggi come nel WebSocket
      if (j.type === "coverage" && j.points) {
        // TODO: aggiorna la mappa come fai in WebBridge
  ...
  mqttClient.on("error", function () {
    ...
    function toggleVideoPanel() {
      ...
      if (videoCollapsed) {
        ...
        // Try to connect to video stream if not already connected
        if (!videoConnected) {
          connectVideoStream();
        }
      }
    ...
    function connectVideoStream() {
      ...
      // Simulate video connection - replace with actual stream URL
      img.src = streamUrl;
      ...
      placeholder.style.display = 'none';
       ...
       img.onerror = function() {
          ...
          // Optional: tenta di ricaricare lo stream dopo 3 secondi
    ...
    function initializeMap() {
      const img = new Image();
      img.src = "complete_house_map_save.png";
      ...
      img.onload = () => {
        ...
        L.imageOverlay("complete_house_map_save.png", bounds).addTo(map);
        map.fitBounds(bounds);

        map.on("click", (e) => {

          // Se c'è una zona selezionata, deselezionala
          if (selectedZone) {
            clearSelectedZone();
          }
  ...
  console.log("Sent Polygon via MQTT:", msg);
    ...
    function drawVirtualZones() {
      ...
      for (let i = 0; i < virtualZonesData.length; i++) {
        ...
        //   selectedZone.polygon.setStyle({ fillOpacity: 0.8 });
    ...
    function editVirtualZone(zoneId) {
      ...
      // Seleziona il colore corretto nella palette
...
function sendManualCmd(command) {
  ...
  function openRuleForm(rule = null) {
    ...
    // Se sto modificando, riempio i valori
    if (rule) {
  ...
  function saveRule() {
    const newRule = {
      id: editingRule ? editingRule.id : ruleIdSeq++,
  ...
  function renderRules() {
    ...
    async function deleteRule(id) {
      ...
      try {
        ...
        // Se il backend risponde correttamente, aggiorna i dati locali
